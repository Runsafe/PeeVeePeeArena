...
public class PlayerScoresRepository extends Repository implements IConfigurationChanged
{
	...
	public HashMap<Integer, List<String>> getSchemaUpdateQueries()
	{
		...
		update.add("ALTER TABLE `peeveepee_scores` ADD COLUMN `rating` INT(5) NOT NULL DEFAULT '1500' AFTER `deaths`, ADD COLUMN `points` INT(10) NOT NULL DEFAULT '0' AFTER `rating`;");
		update.add("ALTER TABLE `peeveepee_scores` CHANGE COLUMN `kills` `kills` int(10) NOT NULL DEFAULT '0' AFTER `playerName`, CHANGE COLUMN `deaths` `deaths` int(10) NOT NULL DEFAULT '0' AFTER `kills`;");
		update.add("UPDATE peeveepee_scores AS s SET rating = (SELECT rating FROM peeveepee_ratings AS r WHERE s.playerName = r.playerName)");
